package io.github.Leonardo0013YT.UltraBreezly.xseries;
...
public enum XMaterial {
     ...
     * @since 1.0.0
     */
    private static final int VERSION = getMajorVersion(Bukkit.getServer().getClass().getPackage().getName().replace(".", ",").split(",")[3]);
    /**
    ...
    public static Optional<XMaterial> matchXMaterial(String material, short data) {
        if (data < 0) return Optional.empty();

        // Looping through Material.values() will take longer.
        for (XMaterial materials : VALUES)
            if (materials.data == data && materials.name().equals(material)) return Optional.of(materials);
        return Optional.empty();
    }
